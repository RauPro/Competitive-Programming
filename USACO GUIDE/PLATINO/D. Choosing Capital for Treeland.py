...
def main():
    ...
    for i in range(n - 1):
        ...
        AL[u].append(v)
        AL[v].append(u)
        direction[u].append(0)
        direction[v].append(1)
    def dfs(u, p):
        ans = 0
        for v in AL[u]:
            if v != p:
                ans += dfs(v,  u) + ((u, v) in direction)
        return ans

    default = [0] * n

    def combine(nodeDP, neiDP, node, eind):
        #print(nodeDP, neiDP, node, eind)
        return nodeDP + neiDP + int(direction[node][eind])

    def finalize(nodeDP, node, eind):
        return nodeDP

    rootDP, forwardDP, reverseDP = rerooter(AL, default, combine, finalize)
    min_inversions = min(rootDP)
    print(min_inversions)
    print(*[(i + 1) for i, it in enumerate(rootDP) if it == min_inversions])
    """min_inversions = INF
    ans= defaultdict(list)
    for i in range(1, n+1):
        local_min = dfs(i, -1)
        ans[local_min].append(i)
        min_inversions = min(min_inversions, local_min)
    print(min_inversions)
    print(*ans[min_inversions])"""
